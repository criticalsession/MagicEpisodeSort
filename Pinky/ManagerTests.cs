...
namespace Pinky
{
    public class ManagerTests
    {
        ...
        public void buildVideoFiles_fromRootDirectory()
        {
            ...
            Assert.AreEqual(Path.Combine(toTest[0].NewDirectory, toTest[0].FileName), Path.Combine(root, "MagicSorted", "A Sample Series", "Season 07", "A.Sample.Series.S07E06.mp4"));
            Assert.AreEqual(Path.Combine(toTest[3].NewDirectory, toTest[3].FileName), Path.Combine(root, "MagicSorted", "A Third Show", "Season 12", "A.Third.Show.S12E0910.episode-name.WEB.x264.mp4"));
            Assert.AreEqual(Path.Combine(toTest[6].NewDirectory, toTest[6].FileName), Path.Combine(root, "MagicSorted", "A Different Series", "Season 07", "A.Different.Series.s07e01.WEBRip.mkv"));
        }
    }
}